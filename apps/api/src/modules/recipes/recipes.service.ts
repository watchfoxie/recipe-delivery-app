...
export class RecipesService {
  ...
  async findAllByIngredient(query: ListRecipesIngredientQueryDto): Promise<PaginatedResult<Recipe>> {
    const page = query.page ?? 1;
    const limit = query.limit ?? 10;

    const parsed = QueryParserUtil.parse(query, {
      sortMapping: RECIPES_SORT_MAPPING,
      filterMapping: RECIPES_INGREDIENT_FILTER_MAPPING,
    });
    ...
    const totalQuery = buildBaseQuery().select('recipe.id').distinct(true);
    QueryParserUtil.applyFilter(totalQuery, parsed.filter);
    const total = await totalQuery.getCount();

    const idQuery = buildBaseQuery().select('recipe.id', 'id').distinct(true);
    QueryParserUtil.applyFilter(idQuery, parsed.filter);

    if (parsed.sort.length) {
      parsed.sort.forEach((descriptor, index) => {
        idQuery.addSelect(descriptor.column, `sort_${index}`);
      });
      QueryParserUtil.applySort(idQuery, parsed.sort);
    } else {
      idQuery.addSelect('recipe.created_at', 'sort_0');
      idQuery.orderBy('recipe.created_at', 'DESC');
    }
