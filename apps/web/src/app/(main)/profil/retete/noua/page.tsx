...
export default function NewRecipePage() {
  ...
  const handleSubmit = async (isPublished: boolean) => {
    ...
    try {
      ...
      success(isPublished ? 'Rețeta a fost publicată cu succes!' : 'Rețeta a fost salvată ca ciornă!');
      router.push('/profil/retete');
    } catch (err) {
      console.error('Error creating recipe:', err);
      error('Nu am putut salva rețeta. Te rugăm să încerci din nou.');
    } finally {
    ...
    <div className="max-w-3xl mx-auto py-8 px-4">
      ...
      <div className="flex gap-4">
        <button
          ...
          className="flex-1 py-3 rounded-lg font-semibold text-brown bg-cream border-2 border-brown hover:bg-cream/80 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
