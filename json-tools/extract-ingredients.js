...
// Recursively walk the JSON structure and gather ingredient objects wherever an `ingredients` array exists.
function collectIngredients(node, seen, conflicts) {
    ...
    if (Array.isArray(node.ingredients)) {
        node.ingredients.forEach((ing) => {
            if (!ing || ing.ingredientId === undefined) {
                console.warn('S-a omis ingredientul fără ingredientId:', ing);
                return;
            }

            const id = Number(ing.ingredientId);
            if (!Number.isFinite(id)) {
                console.warn('S-a omis ingredientul cu ingredientId non-numeric:', ing);
                return;
            }

            const existing = seen.get(id);
            if (!existing) {
                seen.set(id, { ...ing, ingredientId: id });
            } else if (JSON.stringify(existing) !== JSON.stringify(ing)) {
                conflicts.push({ id, existing, incoming: ing });
            }
        });
    }
...
async function main() {
    try {
        ...
        if (conflicts.length > 0) {
            console.warn('Duplicate conflict detectate (primul caz păstrat):');
            conflicts.forEach(({ id }) => {
                console.warn(` - ingredientId ${id}`);
            });
        }
    } catch (err) {
        console.error('Extragerea a esuat:', err);
        process.exitCode = 1;
    }
}
